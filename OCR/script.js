...
start.onclick = () => {
    ...
    rec.recognize(fileSelector.files[0])
        ...
        .then(function (data) {
            ...
            // Save scanned data to a text file
            saveScannedDataToFile(data.text);
        });
};
function saveScannedDataToFile(scannedData) {
    const blob = new Blob([scannedData], { type: 'text/plain;charset=utf-8' });
